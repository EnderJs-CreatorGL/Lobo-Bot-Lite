...
mobile: MethodMobile, 
browser: opcion == '1' ? ['Lobo Bot', 'Safari', '2.0.0'] : methodCodeQR ? ['Lobo Bot', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '110.0.5585.95'],
auth: {
creds: state.creds,
...
rl.close()  
        ...
        setTimeout(async () => {
            ...
            console.log(chalk.black(chalk.bgGreen(`🚩 Código de session: `)), chalk.black(chalk.white(codigo)))
        }, 3000)
}}
}
...
if (!opts['test']) {
  if (global.db) {
    setInterval(async () => {
      ...
      if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', 'LoboJadiBot'], tmp.forEach((filename) => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])));
    }, 30 * 1000);
  }
}
...
try {
let listaDirectorios = readdirSync('./LoboJadiBot/');
let SBprekey = []
listaDirectorios.forEach(directorio => {
if (statSync(`./OlympusJadiBot/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./LoboJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys]
DSBPreKeys.forEach(fileInDir => {
unlinkSync(`./LoboJadiBot/${directorio}/${fileInDir}`)
})
}
})
...
const directories = ['./LoboSession/', './LoboJadiBot/']
...
if (opcion == '1' || methodCodeQR) {
    console.log(chalk.bold.yellow(`\n✅️ ESCANEA ESTE CODIGO QR PARA SER BOT...`))}
}
   if (connection == 'open') {
      console.log(chalk.bold.green('\n🔥 WhatsApp Bot Activado.'));
  }
